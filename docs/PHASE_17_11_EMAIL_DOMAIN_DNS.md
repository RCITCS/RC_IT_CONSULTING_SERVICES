...
Preserve working Cloudflare Email Routing while independently proving the outbound Resend identity for `rcitcs.com` and requiring an explicit DMARC record without deleting unrelated mail DNS.
...
DMARC is not inferred from Resend provider status. It is verified independently from public DNS by the Phase-17.11 workflow.
...
Required DMARC value:

`v=DMARC1; p=none; rua=mailto:dmarc@rcitcs.com; adkim=s; aspf=s; pct=100`

## Activation boundary

The dedicated Phase-17.11 workflow is read-only. Pull-request runs classify the current DMARC state without mutating production DNS. Final `main` requires the DMARC record to be publicly resolvable before Phase 17 can close.
