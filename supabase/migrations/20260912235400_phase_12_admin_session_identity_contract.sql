-- Phase 12 — admin workspace session identity convergence
-- Jobs and Applications consume the nested admin identity from the authenticated
-- session context. Preserve the existing top-level admin_id while also returning
-- the same immutable identifier inside admin.id so every authenticated workspace
-- receives one consistent server-authoritative session shape.
