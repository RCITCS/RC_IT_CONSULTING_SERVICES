...
<nav class="primary-nav" aria-label="Administration"><a href="${basePath || "/"}">${icon("overview")}<span>Overview</span></a><a href="${basePath}/jobs">${icon("jobs")}<span>Jobs</span></a><a href="${basePath}/change-password" aria-current="page">${icon("security")}<span>Security</span></a></nav>
<div class="header-actions"><span class="header-account">${esc(session.admin.email)}</span><form class="header-signout" method="post" action="${basePath}/logout"><input type="hidden" name="csrf" value="${esc(session.csrf)}"><button type="submit" aria-label="Sign out" title="Sign out">${icon("signout")}</button></form><details class="mobile-nav"><summary>Menu</summary><div class="mobile-menu"><a href="${basePath || "/"}">Overview</a><a href="${basePath}/jobs">Jobs</a><a href="${basePath}/change-password" aria-current="page">Security</a><form method="post" action="${basePath}/logout"><input type="hidden" name="csrf" value="${esc(session.csrf)}"><button type="submit">Sign out</button></form></div></details></div>
</div></header>
...
<aside class="side-plane" aria-label="Security status"><section class="side-section" aria-labelledby="identity-title"><div class="side-title"><h2 id="identity-title">Account authority</h2><span>Verified session</span></div><div class="identity-list"><div class="identity-row"><span>Account</span><strong>${esc(session.admin.email)}</strong></div><div class="identity-row"><span>Role</span><strong class="role-text">${esc(session.admin.role)}</strong></div><div class="identity-row"><span>Session expires</span><strong>${esc(prettyTime(session.expires_at))}</strong></div></div></section><section class="side-section" aria-labelledby="controls-title"><div class="side-title"><h3 id="controls-title">Security controls</h3><span>Protected</span></div><div class="readonly-note">${icon("info")}<span>Authentication, password verification, CSRF validation and session revocation are enforced server-side. Recruitment publishing is managed from the Jobs workspace.</span></div><div class="side-actions"><a class="btn secondary" href="${basePath || "/"}">Return to overview</a><a class="btn secondary" href="${basePath}/jobs">Open job management</a></div></section></aside>
</div>
