...
main[aria-labelledby="job-editor-title"] .field input:not([type="checkbox"]),
main[aria-labelledby="job-editor-title"] .field select,
main[aria-labelledby="job-editor-title"] .field textarea{
...
main[aria-labelledby="job-editor-title"] .field input:not([type="checkbox"]),
main[aria-labelledby="job-editor-title"] .field select{min-height:46px;padding:10px 12px}
main[aria-labelledby="job-editor-title"] .field select{-webkit-appearance:none;appearance:none;padding-right:38px;background-image:linear-gradient(45deg,transparent 50%,#667085 50%),linear-gradient(135deg,#667085 50%,transparent 50%);background-position:calc(100% - 17px) 20px,calc(100% - 12px) 20px;background-size:5px 5px,5px 5px;background-repeat:no-repeat}
main[aria-labelledby="job-editor-title"] .field textarea{display:block;min-height:160px;padding:13px 14px;resize:vertical;overflow:auto;white-space:pre-wrap;overflow-wrap:anywhere}
main[aria-labelledby="job-editor-title"] #job-required-skills{min-height:220px}
main[aria-labelledby="job-editor-title"] #job-description{min-height:360px}
main[aria-labelledby="job-editor-title"] #job-benefits{min-height:190px}
main[aria-labelledby="job-editor-title"] #job-summary,
main[aria-labelledby="job-editor-title"] #job-location-details{min-height:150px}
main[aria-labelledby="job-editor-title"] .field input:not([type="checkbox"]):focus,
main[aria-labelledby="job-editor-title"] .field select:focus,
main[aria-labelledby="job-editor-title"] .field textarea:focus{border-color:var(--accent);box-shadow:0 0 0 3px rgba(47,91,211,.10);outline:none}
main[aria-labelledby="job-editor-title"] .field textarea::placeholder,
main[aria-labelledby="job-editor-title"] .field input::placeholder{color:#98a2b3}
main[aria-labelledby="job-editor-title"] .field .muted{margin:8px 0 0;line-height:1.5}
@media(max-width:760px){
  main[aria-labelledby="job-editor-title"] .field input:not([type="checkbox"]),
  main[aria-labelledby="job-editor-title"] .field select,
  main[aria-labelledby="job-editor-title"] .field textarea{font-size:16px}
  main[aria-labelledby="job-editor-title"] .field textarea{min-height:180px;padding:14px}
  main[aria-labelledby="job-editor-title"] #job-required-skills{min-height:240px}
  main[aria-labelledby="job-editor-title"] #job-description{min-height:400px}
  main[aria-labelledby="job-editor-title"] #job-benefits{min-height:220px}
}
</style>`;
...
function categoryOptions(context: JobManagementContext, selected = ""): string {
  ...
  for (const category of context.categories || []) {
    ...
    if (!name || category.is_active === false) continue;
    const key = name.toLocaleLowerCase("en-GB");
    if (!byName.has(key)) byName.set(key, name);
  }
  for (const name of REQUESTED_CATEGORIES) {
    const key = name.toLocaleLowerCase("en-GB");
    if (!byName.has(key)) byName.set(key, name);
  }
  return [...byName.values()].sort((a,b)=>a.localeCompare(b)).map((name)=>option(name,name,selected)).join("");
}
...
function filterJobs(jobs: JobRecord[], query = "", status = ""): JobRecord[] {
  ...
  return jobs.filter((job) => (!status || status === "all" || job.status === status) && (!q || [job.title,job.code,job.category_name,job.location].some((item)=>String(item ?? "").toLowerCase().includes(q))));
}
...
export function jobsListPage(basePath: string, session: AdminSessionView, context: JobManagementContext, query = "", status = "all", message = "", error = false): Response {
  ...
  const rows = visible.length ? visible.map((job) => `<tr>
    <td><strong class="activity-action">${esc(job.title || "Untitled")}</strong><div class="activity-type">${esc(job.code || "Identifier pending")}</div></td>
...
export function jobEditorPage(basePath: string, session: AdminSessionView, context: JobManagementContext, job: JobRecord | null = null, message = "", error = false, submitted?: Record<string, unknown>): Response {
  ...
  return shell(editing ? "Edit job" : "Create job", `${JOB_EDITOR_POLISH}<div class="admin-shell">${adminHeader(basePath, session, "jobs")}${workspaceBar(editing ? "Edit vacancy" : "Create vacancy")}
...
export function jobDeletePage(basePath: string, session: AdminSessionView, job: JobRecord, message = "", error = false): Response {
  ...
  <main class="workspace" id="main-content"><div class="page-heading"><div><div class="eyebrow">Irreversible action</div><h1>Delete ${esc(job.title || "job draft")}</h1><p>Only a never-published draft with zero applications may be permanently deleted.</p></div>${statusBadge(job.status)}</div>${notice(message,error)}
  <section class="data-plane"><div style="padding:22px;max-width:720px">${eligible?`<p style="font-size:12px;line-height:1.65">To confirm permanent deletion, type the exact job code <strong>${esc(code)}</strong>.</p><form method="post" action="${basePath}/jobs/${esc(job.id || "")}/delete"><input type="hidden" name="csrf" value="${esc(session.csrf)}"><input type="hidden" name="expected_version" value="${esc(job.version ?? 1)}"><div class="field"><label for="confirm-code">Confirm job code</label><input id="confirm-code" name="confirm_code" autocomplete="off" required></div><div class="actions"><button class="btn" type="submit">Permanently delete draft</button><a class="btn secondary" href="${basePath}/jobs">Cancel</a></div></form>`:`<div class="msg error">This record does not satisfy the deletion policy.</div><a class="btn secondary" href="${basePath}/jobs">Back to jobs</a>`}</div></section></main></div>`, eligible?200:409);
}
...
export function jobPayloadFromForm(form: FormData): { payload: Record<string, unknown>; errors: string[] } {
  ...
  return { errors, payload: {
    ...
    application_response_window: responseWindow, location_details: locationDetails,
