...
function messageCard(message: any): string {
  ...
  return `<article class="candidate-message" data-message-id="${esc(message?.id || "")}" aria-label="${esc(ariaLabel)}" style="padding:18px 0;border-bottom:1px solid var(--line)">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,210px),1fr));gap:6px 18px;margin-top:10px;font-size:11px">
      <div><span style="color:var(--muted)">From</span><br><strong style="overflow-wrap:anywhere">${esc(sender || "—")}</strong></div>
      <div><span style="color:var(--muted)">To</span><br><strong style="overflow-wrap:anywhere">${esc(recipient || "—")}</strong></div>
    </div>
...
export function renderCandidateMessageComposer(
  ...
  const composeForm = `${templatePicker(basePath, csrf, application)}<form method="post" action="${basePath}/applications/${esc(applicationId)}/message" style="display:grid;gap:12px" aria-describedby="candidate-message-help">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,210px),1fr));gap:10px">
      <div class="identity-row"><span>From</span><strong style="overflow-wrap:anywhere">careers@rcitcs.com</strong></div>
      <div class="identity-row"><span>To</span><strong style="overflow-wrap:anywhere">${esc(recipient || "Persisted candidate email unavailable")}</strong></div>
    </div>
    <p id="candidate-message-help" class="muted" style="margin:0;font-size:10px">Recipient, sender, reply-to and provider are server-controlled. Preview is mandatory before final confirmation.</p>
  ...
  const confirmation = `<section aria-labelledby="candidate-message-preview-title" style="margin-top:18px;border-top:1px solid var(--line);padding-top:18px">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,210px),1fr));gap:8px 18px;font-size:11px;margin-bottom:12px"><div><span style="color:var(--muted)">From</span><br><strong style="overflow-wrap:anywhere">careers@rcitcs.com</strong></div><div><span style="color:var(--muted)">To</span><br><strong style="overflow-wrap:anywhere">${esc(recipient)}</strong></div></div>
    ...
    <form method="post" action="${basePath}/applications/${esc(applicationId)}/message" style="margin-top:14px" aria-label="Confirm candidate email delivery">
      <input type="hidden" name="csrf" value="${esc(csrf)}">
      <input type="hidden" name="request_id" value="${esc(requestId)}">
...
export function renderCandidateStatusWorkflow(basePath: string, csrf: string, application: any): string {
  ...
  return `<form method="post" action="${basePath}/applications/${esc(applicationId)}/status" style="display:grid;gap:10px" aria-describedby="candidate-stage-help">
    <input type="hidden" name="csrf" value="${esc(csrf)}">
    ...
    <p id="candidate-stage-help" class="muted" style="margin:0;font-size:10px"><strong>Status and email are separate operations.</strong> Updating the recruitment stage never sends candidate email automatically.</p>
