...
assert.ok(indexSource.includes('ADMIN_PROXY_HEADER = "x-rcitcs-admin-proxy"'));
assert.ok(indexSource.includes('ADMIN_PROXY_VALUE = "cloudflare"'));
assert.ok(indexSource.includes('"https://admin.rcitcs.com"'));
assert.ok(indexSource.includes('"https://admin-staging.rcitcs.com"'));
assert.ok(indexSource.includes('function browserNavigationPostOk'));
assert.ok(indexSource.includes('request.headers.get("sec-fetch-site") === "same-origin"'));
assert.ok(indexSource.includes('request.headers.get("sec-fetch-mode") === "navigate"'));
assert.ok(indexSource.includes('request.headers.get("sec-fetch-dest") === "document"'));
assert.ok(indexSource.includes('request.headers.get("sec-fetch-user") === "?1"'));
assert.ok(indexSource.includes('if (!origin || origin === "null") return true'));
assert.ok(indexSource.includes('if (!origin || origin === "null") return false'));
...
assert.ok(databaseSource.includes('if (response.ok)'));
assert.ok(databaseSource.includes('return context?.session ?? null;'));
...
console.log('PASS: Phase 9 cryptography helpers, secure-cookie/session controls, throttling, dual-layer browser POST validation, resilient server-authoritative session lookup, CSRF, generic recovery responses, secret-key handling and migration isolation remain verified.');
