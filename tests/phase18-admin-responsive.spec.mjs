...
// Production keeps a defensive document clip. Remove only that clip in the QA harness so
// document-level overflow remains observable and component containment must stand on its own.
const ADMIN_RESPONSIVE_TEST_STYLE = ADMIN_RESPONSIVE_STYLE.replace(
  'html,body{max-width:100%;overflow-x:hidden}',
  'html,body{max-width:100%}'
);

function adminHarness() {
  return `<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1">${BASE_ADMIN_STYLE}${ADMIN_RESPONSIVE_TEST_STYLE}</head><body>
...
test('admin authentication surface fits phone and tablet viewports', async ({ page }) => {
  await page.setContent(`<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1">${BASE_ADMIN_STYLE}${ADMIN_RESPONSIVE_TEST_STYLE}</head><body><div class="auth"><aside class="auth-context"><div class="auth-brand">RC IT Services</div><div class="auth-context-copy">Private administration</div></aside><main class="auth-main"><form class="auth-card"><h1>Administrator sign in</h1><div class="field"><label>Email<input type="email" value="administrator@example.com"></label></div><div class="field"><label>Password<input type="password" value="example-password"></label></div><button class="btn">Sign in</button></form></main></div></body></html>`);
...
test('admin action dialog remains bounded and internally scrollable', async ({ page }) => {
  await page.setContent(`<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1">${BASE_ADMIN_STYLE}${ADMIN_RESPONSIVE_TEST_STYLE}${ADMIN_INTERACTION_STYLE}</head><body><dialog class="rc-admin-dialog" open><div class="rc-admin-dialog-shell"><header class="rc-admin-dialog-bar"><h2>Very long administrative action title that must remain contained</h2><button class="rc-admin-dialog-close" type="button">×</button></header><div class="rc-admin-dialog-body"><main class="workspace"><div class="page-heading"><div><h1>Edit vacancy</h1></div></div><section class="data-plane"><form class="job-editor-form" style="display:grid;grid-template-columns:1fr 1fr;gap:16px"><label>Title<input value="Principal Platform Engineering Architect"></label><label>Location<input value="London"></label><label style="grid-column:1/-1">Description<textarea id="job-description">Long form content</textarea></label><div class="actions" style="grid-column:1/-1"><button class="btn">Save</button><button class="btn secondary">Cancel</button></div></form></section></main></div></div></dialog></body></html>`);
