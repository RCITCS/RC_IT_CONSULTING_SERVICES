...
function rcAdminInteractionsBootstrap() {
  ...
  function announce(message) {
    ...
    if (!live) {
      ...
      live.className = 'rc-admin-live-region';
  ...
  function syncNavigationState(url) {
    const route = url.pathname.startsWith('/jobs') ? 'jobs'
      : url.pathname.startsWith('/applications') ? 'applications'
      : (url.pathname === '/security' || url.pathname === '/change-password') ? 'security'
      : 'overview';
    for (const link of document.querySelectorAll('.primary-nav a[href],.mobile-menu a[href]')) {
      const target = sameOriginUrl(link.href);
      if (!target) continue;
      const targetRoute = target.pathname.startsWith('/jobs') ? 'jobs'
        : target.pathname.startsWith('/applications') ? 'applications'
        : (target.pathname === '/security' || target.pathname === '/change-password') ? 'security'
        : target.pathname === '/' ? 'overview' : '';
      if (targetRoute === route) link.setAttribute('aria-current', 'page');
  ...
  function applyTopLevelDocument(nextDocument, url, { historyMode = 'push', scrollY = 0 } = {}) {
    ...
    const nextShell = nextDocument.querySelector('.admin-shell');
    const currentMain = document.querySelector('main.workspace');
    const nextMain = nextDocument.querySelector('main.workspace');
    const currentBar = document.querySelector('.workspace-bar');
    const nextBar = nextDocument.querySelector('.workspace-bar');

    const update = () => {
      document.title = nextDocument.title || document.title;
      if (currentShell && nextShell && currentMain && nextMain) {
        if (currentBar && nextBar) currentBar.replaceWith(nextBar.cloneNode(true));
  ...
  function ensureDialog() {
    ...
    dialog.className = 'rc-admin-dialog';
    dialog.setAttribute('aria-labelledby', 'rc-admin-dialog-title');
    ...
    dialog.addEventListener('close', () => {
      ...
      const shouldRefresh = modalDirty && !skipCloseRefresh;
      activeDialog = null;
      activeTrigger = null;
      modalDirty = false;
  ...
  function renderModalDocument(nextDocument, url, trigger = null) {
    ...
    const body = dialog.querySelector('.rc-admin-dialog-body');
    const title = dialog.querySelector('#rc-admin-dialog-title');
    const workspace = nextDocument.querySelector('main.workspace');
    const authCard = nextDocument.querySelector('.auth-card');
    const source = workspace || authCard || nextDocument.querySelector('main#main-content');
    if (!source) throw new Error('Admin modal content unavailable');
    const heading = source.querySelector('h1');
    title.textContent = heading?.textContent?.trim() || 'Admin action';
    body.innerHTML = workspace ? workspace.innerHTML : source.innerHTML;
    dialog.dataset.rcModalUrl = url.href;
    for (const link of body.querySelectorAll('a[href]')) {
      const target = sameOriginUrl(link.href);
      if (target && isDocumentPath(target.pathname)) {
        link.target = '_blank';
        link.rel = 'noopener noreferrer';
        link.dataset.rcHardDocument = 'true';
      }
    }
  ...
  async function softNavigate(url, { historyMode = 'push', preserveScroll = false, restoreScroll = null } = {}) {
    ...
    try {
      ...
      if (!isSoftGetPath(result.url.pathname) || isModalPath(result.url.pathname)) {
        location.assign(result.url.href);
        return;
      }
      if (activeDialog?.open) {
        skipCloseRefresh = true;
        activeDialog.close();
      }
  ...
  async function openModal(url, trigger = null) {
    modalController?.abort();
    modalController = new AbortController();
    const dialog = ensureDialog();
    const body = dialog.querySelector('.rc-admin-dialog-body');
    body.innerHTML = '<div class="rc-admin-dialog-loading" role="status">Loading…</div>';
    if (trigger) activeTrigger = trigger;
    if (!dialog.open) dialog.showModal();
    try {
      const result = await requestHtml(url.href, {}, modalController);
      if (!isModalPath(result.url.pathname)) {
        skipCloseRefresh = true;
        dialog.close();
        await softNavigate(result.url, { historyMode: 'replace' });
        return;
      }
  ...
  async function refreshCurrentView({ preserveScroll = true } = {}) {
    ...
    if (!isSoftGetPath(url.pathname) || isModalPath(url.pathname)) return;
    const y = preserveScroll ? window.scrollY : 0;
    const result = await requestHtml(url.href);
    applyTopLevelDocument(result.document, url, { historyMode: 'none', scrollY: y });
  }
  ...
  async function softSubmit(form, submitter) {
    const action = sameOriginUrl(form.action || location.href);
    if (!action) return;
    const method = (form.method || 'get').toLowerCase();
    if (method === 'get') {
      const params = formBody(form, submitter);
      action.search = params.toString();
      await softNavigate(action, { historyMode: 'push' });
      return;
    }
    if (method !== 'post' || hardPostPaths.has(action.pathname) || !form.querySelector('input[name="csrf"]')) {
      form.requestSubmit(submitter || undefined);
      return;
    }

    const insideModal = Boolean(form.closest('.rc-admin-dialog'));
    ...
    try {
      ...
      const result = await requestHtml(action.href, {
        method: 'POST',
        body,
        headers: {
      ...
      const authError = result.document.querySelector('main.auth');
      if (insideModal && authError) {
        renderModalDocument(result.document, result.url, activeTrigger);
        return;
      }
      if (insideModal) {
        if (isModalPath(result.url.pathname)) {
          ...
          skipCloseRefresh = true;
          activeDialog?.close();
          await refreshCurrentView({ preserveScroll: true });
        }
        return;
      }
      if (isModalPath(result.url.pathname)) {
        modalDirty = true;
        renderModalDocument(result.document, result.url, button || form);
        return;
      }
      if (result.url.pathname === location.pathname && location.pathname === '/jobs') {
        await refreshCurrentView({ preserveScroll: true });
        return;
      }
  ...
  document.addEventListener('click', (event) => {
    ...
    const close = event.target.closest('[data-rc-modal-close]');
    if (close && activeDialog?.open) {
    ...
    if (activeDialog?.open && link.closest('.rc-admin-dialog') && topLevelPaths.has(url.pathname)) {
      ...
      if ((location.pathname === '/jobs' && url.pathname === '/jobs') || (location.pathname === '/applications' && url.pathname === '/applications')) activeDialog.close();
        ...
        skipCloseRefresh = true;
        activeDialog.close();
        softNavigate(url, { historyMode: 'push' });
      }
      return;
    }
    if (isModalPath(url.pathname)) {
      event.preventDefault();
      openModal(url, link);
      return;
    }
    if (topLevelPaths.has(url.pathname)) {
      event.preventDefault();
      softNavigate(url, { historyMode: 'push' });
    }
  });
  ...
  document.addEventListener('submit', (event) => {
    ...
    if (!action) return;
    const method = (form.method || 'get').toLowerCase();
    if (method === 'get' && isSoftGetPath(action.pathname) && !isModalPath(action.pathname)) {
      event.preventDefault();
      softSubmit(form, event.submitter).catch(() => {});
      return;
    }
    if (method === 'post' && !hardPostPaths.has(action.pathname) && form.querySelector('input[name="csrf"]')) {
      event.preventDefault();
  ...
  window.addEventListener('popstate', (event) => {
    ...
    const url = new URL(location.href);
    if (topLevelPaths.has(url.pathname)) softNavigate(url, { historyMode: 'none', restoreScroll: Number(event.state?.scrollY || 0) });
  });

  history.replaceState({ ...(history.state || {}), rcAdmin: true, scrollY: window.scrollY }, '', location.href);
  syncNavigationState(new URL(location.href));
}
